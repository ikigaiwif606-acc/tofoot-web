...
export default function NextMatchBanner() {
  ...
  if (diff <= 0 || diff > 7 * 24 * 60 * 60 * 1000) return null; // Only show within 7 days
    ...
    <div className="border-b border-accent/20 bg-accent/5">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-2 sm:px-6">
        <div className="flex items-center gap-3 text-sm">
          <span className="animate-pulse-soft rounded-full bg-accent/20 px-2 py-0.5 text-xs font-bold text-accent">
            NEXT
          </span>
        ...
        <Link
          ...
          className="rounded-md bg-accent/10 px-3 py-1 text-xs font-medium text-accent transition-colors hover:bg-accent/20"
        >
          去預測
        </Link>
      </div>
